...
#this file has been altered to receive values from the magnetometer sensor
...
def getXY():                                        # this function returns an average of several magnetometer readings for x and y
    data = np.take(mag.magRead(), [0, 1])            # take only the first two elements of the returned array
    for i in range(10):                             # iterate 10 times (i will start at zero)
        newData = np.take(mag.magRead(), [0, 1])     # call getMag and take the first two elements
...
def whereismyHead():      #function for calling scaled mag values from other programs
